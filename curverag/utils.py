...
def create_atth_dataset(graph):
  nodes_idx = {n.name: i for i, n in enumerate(graph.nodes)}
  nodes_idx = {n.id: i for i, n in enumerate(graph.nodes)}
  edges_ = set([e.name for e in graph.edges])
  edges = {e for i, e in enumerate(edges_)}
  relationship_idx = {n: i for i, n in enumerate(edges_)}

  graph_list = []
  for e in graph.edges:
    graph_list.append([nodes_idx[e.source], relationship_idx[e.name], nodes_idx[e.target]])
...
def save_kg_dataset(triples, output_dir):
